...
class UNetSingleInstrument(nn.Module):
    ...
    def forward(self, x):
        ...
        # Encoder path
        # For K=5, S=2, TensorFlow 'SAME' padding is (1,2) for H and W
        # F.pad format: (pad_left, pad_right, pad_top, pad_bottom)
        # PyTorch input (B, C, H, W) -> (B, C, T, F)
        # So padding is for F (W) then T (H) dimensions: (1,2,1,2)
        x_pad1 = F.pad(x, (1, 2, 1, 2), mode='constant', value=0) # Changed mode to constant zero-padding
        conv1 = self.encoder_conv1(x_pad1)
        batch1 = self.encoder_bn1(conv1)
        rel1 = self.leaky_relu(batch1)
        print(f"Step1 (conv1): {conv1.shape} ({conv1.min():.6f}, {conv1.max():.6f})")
        ...
        conv2 = self.encoder_conv2(rel1_pad2)
        batch2 = self.encoder_bn2(conv2)
        rel2 = self.leaky_relu(batch2)
        print(f"Step2 (conv2): {conv2.shape} ({conv2.min():.6f}, {conv2.max():.6f})")
        print(f"Step2 (after bn+relu): {rel2.shape} ({rel2.min():.6f}, {rel2.max():.6f})")
        
        rel2_pad3 = F.pad(rel2, (1, 2, 1, 2), mode='constant', value=0)
        conv3 = self.encoder_conv3(rel2_pad3)
        batch3 = self.encoder_bn3(conv3)
        rel3 = self.leaky_relu(batch3)
        print(f"Step3 (conv3): {conv3.shape} ({conv3.min():.6f}, {conv3.max():.6f})")
        print(f"Step3 (after bn+relu): {rel3.shape} ({rel3.min():.6f}, {rel3.max():.6f})")
        
        rel3_pad4 = F.pad(rel3, (1, 2, 1, 2), mode='constant', value=0)
        conv4 = self.encoder_conv4(rel3_pad4)
        batch4 = self.encoder_bn4(conv4)
        rel4 = self.leaky_relu(batch4)
        print(f"Step4 (conv4): {conv4.shape} ({conv4.min():.6f}, {conv4.max():.6f})")
        print(f"Step4 (after bn+relu): {rel4.shape} ({rel4.min():.6f}, {rel4.max():.6f})")
        
        rel4_pad5 = F.pad(rel4, (1, 2, 1, 2), mode='constant', value=0)
        conv5 = self.encoder_conv5(rel4_pad5)
        batch5 = self.encoder_bn5(conv5)
        rel5 = self.leaky_relu(batch5)
        print(f"Step5 (conv5): {conv5.shape} ({conv5.min():.6f}, {conv5.max():.6f})")
        print(f"Step5 (after bn+relu): {rel5.shape} ({rel5.min():.6f}, {rel5.max():.6f})")
        
        rel5_pad6 = F.pad(rel5, (1, 2, 1, 2), mode='constant', value=0)
        conv6 = self.encoder_conv6(rel5_pad6)
        print(f"Step6 (conv6): {conv6.shape} ({conv6.min():.6f}, {conv6.max():.6f})")
        
        # Decoder path with skip connections
        up1 = self.decoder_conv1(conv6)
        up1 = self.relu(up1)
        batch7 = self.decoder_bn1(up1)
        drop1 = self.dropout(batch7)
        merge1 = torch.cat([conv5, drop1], dim=1)
        ...
        merge2 = torch.cat([conv4, drop2], dim=1)
